...
describe('AddMappingDialog', () => {
  it('renders source and target inputs', () => {
    render(<AddMappingDialog groupId={GROUP_ID} onConfirm={vi.fn()} onCancel={vi.fn()} />);
  ...
  it('shows validation error for out-of-range port', () => {
    ...
    render(<AddMappingDialog groupId={GROUP_ID} onConfirm={onConfirm} onCancel={vi.fn()} />);
  ...
  it('calls onCancel when Cancel clicked', () => {
    ...
    render(<AddMappingDialog groupId={GROUP_ID} onConfirm={vi.fn()} onCancel={onCancel} />);
  ...
  it('includes name when provided', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /^add$/i }));
    expect(onConfirm).toHaveBeenCalledWith(
      expect.objectContaining({ name: 'my-svc', sourcePort: 9090, targetHost: '0.0.0.0', targetPort: 4000, groupId: GROUP_ID }),
    );
  });

  it('calls onCancel when overlay clicked', () => {
    const onCancel = vi.fn();
    const { container } = render(<AddMappingDialog groupId={GROUP_ID} onConfirm={vi.fn()} onCancel={onCancel} />);
  ...
  it('renders in edit mode with prefilled values and Save button', () => {
    render(<AddMappingDialog groupId={GROUP_ID} initial={sampleMapping} onConfirm={vi.fn()} onCancel={vi.fn()} />);
  ...
  it('emits edited values from Save', () => {
    const onConfirm = vi.fn();
    render(<AddMappingDialog groupId={GROUP_ID} initial={sampleMapping} onConfirm={onConfirm} onCancel={vi.fn()} />);
  ...
  it('Enter key submits the form', () => {
    const onConfirm = vi.fn();
    const { container } = render(<AddMappingDialog groupId={GROUP_ID} onConfirm={onConfirm} onCancel={vi.fn()} />);
    fireEvent.change(screen.getByPlaceholderText('8080'), { target: { value: '8080' } });
    fireEvent.change(screen.getByPlaceholderText('localhost:3000'), { target: { value: 'localhost:3000' } });
